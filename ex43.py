...
class LaserWeaponArmory(Scene):
	...
	def enter(self):
		...
		while guess not in [code, cheat] and guesses <9: 
			print "BZZZZEDDD!"
			guesses += 1
			guess = raw_input("[keypad]> ")

		if guess in [code, cheat]:
...
class TheBridge(Scene):
	...
	def enter(self):
		enemy_hp = randint(6,9)
		my_hp = 9
		print("Your hp is %d. Your enemy's hp is %d." % (my_hp, enemy_hp))
		
		hitting = randint(1,4)
		getting_hit = randint(1,5)

		hit = raw_input("> ")

		if hit == "hit":
			print("You hit the enemy for %s." % hitting)
			enemy_hp = enemy_hp - hitting
			if enemy_hp <= 0:
				...
				print("Enemy hits you for %s." % getting_hit)
				my_hp = my_hp - getting_hit
				if my_hp <= 0:
					return 'death'
